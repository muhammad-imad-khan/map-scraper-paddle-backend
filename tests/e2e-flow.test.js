/**
 * End-to-End Test Suite: Customer â†’ Admin â†’ Course Access
 * 
...
async function runTests() {
  ...
  await test('Verify course details accessible in enrollment', async () => {
    ...
    if (enrollments.length > 0) {
      ...
      log('INFO', `Sample enrollment verified: ${enrollment.email} â†’ ${enrollment.courseId}`, 'info');
    }
  });
  ...
  console.log(`âŒ Failed:  ${testResults.failed}`);
