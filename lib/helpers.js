// â”€â”€ Shared Paddle helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function readModeEnv(baseName, fallback = '') {
  ...
  const prefixedKey = IS_PRODUCTION ? `LIVE_${baseName}` : `TEST_${baseName}`;
  const raw = process.env[modePrimary] ?? process.env[modeAlias] ?? process.env[prefixedKey] ?? process.env[baseName] ?? fallback;
...
const PRICE_IDS = {
  ...
  course: readModeEnv('PRICE_COURSE_ID', '') || (IS_PRODUCTION ? String(process.env.LIVE_LEAD_GEN_COURSE_PRICE_ID || '').trim() : '') || DEFAULT_PRICE_IDS.course,
  courseIntl: readModeEnv('PRICE_COURSE_INTL_ID', '') || readModeEnv('PRICE_COURSE_ID', '') || (IS_PRODUCTION ? String(process.env.LIVE_LEAD_GEN_COURSE_PRICE_ID || '').trim() : '') || DEFAULT_PRICE_IDS.course,
  checkoutFallback: readModeEnv('PRICE_CHECKOUT_FALLBACK_ID', ''),
};

const FRONTEND_URL = readModeEnv('FRONTEND_URL', 'https://lead-genx.vercel.app');
const SITE_ORIGIN = readModeEnv('SITE_ORIGIN', FRONTEND_URL || 'https://lead-genx.vercel.app');
const PAYMENT_PAGE_URL = `${(FRONTEND_URL || 'https://lead-genx.vercel.app').replace(/\/$/, '')}/payment/`;
const TOOL_LINK = readModeEnv('TOOL_LINK', '');

const PADDLE_ENV = readModeEnv('PADDLE_ENV', IS_PRODUCTION ? 'production' : 'sandbox').toLowerCase();
const PADDLE_WEBHOOK_SECRET = (IS_PRODUCTION
  ? String(process.env.LIVE_WEBHOOK_SECRET || '').trim()
  : String(process.env.TEST_PADDLE_WEBHOOK_SECRET || '').trim()
) || readModeEnv('PADDLE_WEBHOOK_SECRET', '');
...
// Two paid tiers â€” maps priceId â†’ credits + label
const PRICE_CREDITS = {
...
const INSTALL_ENTITLEMENT_CHECK_TTL_MS = 6 * 60 * 60 * 1000;

// â”€â”€ Redis singleton â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
let _redis;
function getRedis() {
  if (!_redis) {
...
// â”€â”€ Key helpers (RLS: each user scoped by installId) â”€â”€â”€â”€â”€â”€
const keys = {
...
// â”€â”€ Credit operations (all server-side, atomic) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function getCredits(installId) {
  const redis = getRedis();
  // Check expiry first â€” if expired, wipe credits
...
async function loadInstallData(redis, installId) {
  const raw = await redis.get(keys.install(installId));
  const parsed = safeParse(raw, null);
  if (parsed && typeof parsed === 'object') return parsed;
  return {};
}

async function saveInstallData(redis, installId, installData) {
  await redis.set(keys.install(installId), JSON.stringify(installData));
  return installData;
}

function isUnlimitedPurchaseRecord(purchase, installId = '') {
  ...
  if (normalizedInstallId) {
    const purchaseInstallId = String(purchase.installId || '').trim();
    if (!purchaseInstallId || purchaseInstallId !== normalizedInstallId) {
      return false;
    }
  }

  const lifetimePriceIds = getLifetimePriceIds();
  return purchase.unlimited === true
    || lifetimePriceIds.has(String(purchase.priceId || '').trim())
    || String(purchase.label || '').toLowerCase().includes('lifetime');
}

async function findUnlimitedPurchaseByEmail(redis, email, installId) {
  ...
  const purchase = purchases.find((entry) => isUnlimitedPurchaseRecord(entry, installId));
...
async function scanUsersForUnlimitedInstall(redis, installId) {
  if (typeof redis.scan !== 'function') return null;

  let cursor = '0';
  do {
    const reply = await redis.scan(cursor, 'MATCH', 'user:*', 'COUNT', 200);
    const nextCursor = Array.isArray(reply) ? String(reply[0] || '0') : '0';
    const userKeys = Array.isArray(reply?.[1]) ? reply[1] : [];
    ...
    for (const userKey of userKeys) {
      ...
      if (purchase) {
        return {
          email: sanitizeText(userData?.email || userKey.replace(/^user:/, ''), 254).toLowerCase(),
          name: sanitizeText(userData?.name || '', 100),
          purchase,
        };
      }
...
async function getInstallEntitlement(installId, { redis = getRedis(), forceRefresh = false } = {}) {
  const normalizedInstallId = String(installId || '').trim();
  if (!isValidInstallId(normalizedInstallId)) {
    return { unlimited: false, installData: null };
  }

  const installData = await loadInstallData(redis, normalizedInstallId);
  if (!installData.createdAt) {
    installData.createdAt = new Date().toISOString();
  }

  if (installData.unlimited === true && !forceRefresh) {
    return { unlimited: true, installData };
  }

  const lastCheckedAt = Number(installData.entitlementCheckedAt || 0);
  const hasFreshNegativeCache = !forceRefresh
    && installData.unlimited !== true
    && lastCheckedAt > 0
    && (Date.now() - lastCheckedAt) < INSTALL_ENTITLEMENT_CHECK_TTL_MS;

  if (hasFreshNegativeCache) {
    return { unlimited: false, installData };
  }

  let matchedEntitlement = await findUnlimitedPurchaseByEmail(redis, installData.email, normalizedInstallId);
  if (!matchedEntitlement) {
    matchedEntitlement = await scanUsersForUnlimitedInstall(redis, normalizedInstallId);
  }

  installData.entitlementCheckedAt = Date.now();
  if (matchedEntitlement) {
    installData.unlimited = true;
    if (!installData.email && matchedEntitlement.email) {
      installData.email = matchedEntitlement.email;
    }
    if (!installData.name && matchedEntitlement.name) {
      installData.name = matchedEntitlement.name;
    }
    installData.unlimitedActivatedAt = installData.unlimitedActivatedAt || new Date().toISOString();
  }

  await saveInstallData(redis, normalizedInstallId, installData);
  return { unlimited: Boolean(installData.unlimited), installData };
}

async function markInstallUnlimited(installId, { redis = getRedis(), email = '', name = '' } = {}) {
  const normalizedInstallId = String(installId || '').trim();
  if (!isValidInstallId(normalizedInstallId)) return null;
  ...
  const normalizedEmail = sanitizeText(email || '', 254).toLowerCase();
  if (normalizedEmail) installData.email = normalizedEmail;
  if (name) installData.name = sanitizeText(name, 100);
...
// â”€â”€ Validate installId format â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function cors(res) {
  ...
  res.setHeader('Access-Control-Allow-Headers', 'Content-Type, Accept, X-Install-Id, X-Client-Id, X-Admin-Key, Authorization');
}
...
// â”€â”€ Email notifications â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
const ADMIN_EMAIL = readModeEnv('ADMIN_EMAIL', 'mapscrapper7@gmail.com');
const APP_BRAND = 'LEAD GEN x AI Powered Tool';
const APP_BRAND_PRO = 'LEAD GEN x AI Powered Tool Pro';
...
async function sendZipDeliveryEmail({ email, name, txnId }) {
  if (!email) {
    console.warn('Recipient missing, skipping tool access email');
    return { ok: false, reason: 'recipient_missing' };
  }

  const toolLink = String(TOOL_LINK || '').trim();
  if (!toolLink) {
    console.error('TOOL_LINK env variable not set, cannot send tool access email');
    return { ok: false, reason: 'tool_link_missing' };
  }
  ...
  const html = `
    <div style="font-family:Segoe UI,Arial,sans-serif;max-width:560px;margin:0 auto;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;">
      ...
      <div style="padding:24px;">
        ...
        <p style="font-size:14px;color:#334155;margin:0 0 14px;">Your Google Drive tool link is ready below. Open it, download the extension files, and follow the setup steps in this email.</p>
        <div style="text-align:center;margin:20px 0;">
          <a href="${toolLink}" style="display:inline-block;background:#16a34a;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:700;font-size:14px;">Open Google Drive Tool Link</a>
        </div>
        <div style="margin:22px 0 0;border:1px solid #dcfce7;border-radius:12px;background:#f0fdf4;padding:18px;">
          <div style="font-size:12px;color:#166534;font-weight:700;text-transform:uppercase;letter-spacing:1px;margin:0 0 12px;">Setup Steps</div>
          <ol style="margin:0;padding-left:18px;color:#334155;font-size:14px;line-height:1.75;">
            <li>Open the Google Drive link and download the extension files to your computer.</li>
            <li>If the download is a ZIP file, extract it to a permanent folder first.</li>
            <li>Open your browser extensions page: <strong>chrome://extensions</strong> for Chrome, <strong>edge://extensions</strong> for Edge, or <strong>brave://extensions</strong> for Brave.</li>
            <li>Enable <strong>Developer mode</strong> in the top-right corner.</li>
            <li>Click <strong>Load unpacked</strong> and select the extracted extension folder.</li>
            <li>Pin the extension, sign in if needed, and start scraping.</li>
          </ol>
        </div>
        <p style="font-size:13px;color:#475569;margin:18px 0 0;line-height:1.7;">If you need another copy later, return to the payment page while signed in and use the resend email option.</p>
  ...
  try {
    const result = await sendTransactionalEmail({
      from: getMailFrom(APP_BRAND) || process.env.SMTP_USER,
      to: email,
      subject: `Your ${APP_BRAND} tool access link`,
      html,
    });
    console.log(`Tool access email sent to ${email} (txn: ${txnId})`);
    return { ok: true, provider: result?.provider || 'smtp' };
  } catch (err) {
    console.error('Failed to send tool access email:', err.message);
...
async function sendLowCreditsEmail({ email, name, credits, installId }) {
    ...
    <div style="font-family:Segoe UI,Arial,sans-serif;max-width:560px;margin:0 auto;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden;">
      ...
      <div style="padding:24px;">
        ...
        <table style="width:100%;border-collapse:collapse;font-size:13px;">
          <tr><td style="padding:6px 0;color:#64748b;">Pro Pack</td><td style="padding:6px 0;font-weight:600;">500 credits â€” $5</td></tr>
          <tr><td style="padding:6px 0;color:#64748b;">Enterprise Pack</td><td style="padding:6px 0;font-weight:600;">2,500 credits â€” $25</td></tr>
        </table>
      </div>
      <div style="background:#f8fafc;padding:14px 24px;font-size:12px;color:#94a3b8;text-align:center;">
  ...
  try {
    await sendTransactionalEmail({
      from: getMailFrom(APP_BRAND) || process.env.SMTP_USER,
      to: email,
      subject: `Only ${credits} credit${credits !== 1 ? 's' : ''} left â€” top up now`,
      html,
    });
...
// â”€â”€ Course delivery + Google Drive sharing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const CUSTOMER_PORTAL_URL = readModeEnv('CUSTOMER_PORTAL_URL', 'https://lead-genx.vercel.app/portal/');
...
async function deliverToolPurchase({
  ...
  let label = sanitizeText(
    purchase?.label || priceEntry?.label || (grantsUnlimited ? 'Lifetime License' : 'Credit Pack'),
    120,
  ) || 'Credit Pack';

  if (!purchase && !effectivePriceId && !seedPurchase) {
    return { ok: false, entitlementGranted: false, reason: 'purchase_not_found' };
  }

  // Use caller-provided purchase seed (e.g. from admin bank-transfer approve)
  if (!purchase && seedPurchase && typeof seedPurchase === 'object') {
    purchase = {
      ...
      credits: Number(seedPurchase.credits) || 0,
      label: sanitizeText(seedPurchase.label || 'Lifetime License', 120),
      unlimited: Boolean(seedPurchase.unlimited),
      amount,
      currency,
      source,
    };
    userData.purchases.push(purchase);
    // Recompute after seeding
    grantsUnlimited = Boolean(purchase.unlimited);
  ...
  if (grantsUnlimited) {
    await markInstallUnlimited(normalizedInstallId, {
      redis,
      email: normalizedEmail,
      name: userData.name,
    });
...
module.exports = {
  ...
  getInstallEntitlement, markInstallUnlimited,
